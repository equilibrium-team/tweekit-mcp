...
Environment variables `TWEAKIT_API_KEY` and `TWEAKIT_API_SECRET` are used as
...
def resolve_credentials(args: argparse.Namespace) -> tuple[str, str]:
    ...
    if not api_key:
        api_key = prompt_value("Enter TWEAKIT_API_KEY: ")
    if not api_secret:
        api_secret = prompt_value("Enter TWEAKIT_API_SECRET: ", secret=True)
...
def save_credentials(path: Path, api_key: str, api_secret: str) -> None:
    try:
        ...
        content = (
            ...
            f"TWEAKIT_API_KEY={api_key}\n"
            f"TWEAKIT_API_SECRET={api_secret}\n"
        )
